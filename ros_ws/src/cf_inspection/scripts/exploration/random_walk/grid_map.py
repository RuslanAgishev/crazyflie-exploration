...
class GridMap:
    def __init__(self, flight_area_vertices, map_widthX_m=4.0, map_lengthY_m=4.0):
        self.map_center = np.array([0.0, 0.0])
        self.map_width_X_m = map_widthX_m
        self.map_length_Y_m = map_lengthY_m
    ...
    def create_borders_grid_map(self):
        WIDTH = int((self.map_width_X_m) / self.map_resolution_m)
        LENGTH = int((self.map_length_Y_m) / self.map_resolution_m)
        ...
        for i in range(len(points)-1):
            ...
            for l in line:
                gmap[l[1]][l[0]] = 0
        gmap = flood_fill([centroid[1], centroid[0]], gmap)
        self.gmap = gmap
    ...
    def meters2grid(self, pose_m,):
        ...
        # [1, 0](m) -> [100+100, 100]
        # [0,-1](m) -> [100, 100-100]
        nrows = int(self.map_width_X_m / self.map_resolution_m)
        ncols = int(self.map_length_Y_m / self.map_resolution_m)
    ...
    def draw_map(self):
        ax = plt.gca()
        w = self.map_width_X_m; l = self.map_length_Y_m
        ax.set_xlim([-w/2, w/2])
        ax.set_ylim([-l/2, l/2])
